...
public:
    ...
    PyObject* toPyObject(void) const
    {
        PyObject* retVal = NULL;

        PyObject* left = NULL;
        if (__left_) {
            left = __left_->toPyObject();
        } else {
            left = Py_BuildValue("");
        }
        ...
        PyObject* right = NULL;
        if (__right_) {
            right = __right_->toPyObject();
        } else {
            right = Py_BuildValue("");
        }

        char* name = (char*) calloc(size() + 10, 1);
        if (name) {
            get_demangled_name(name);

            int status = -1;
            char* type = __cxa_demangle(typeid(*this).name(), NULL, NULL, &status); // the irony
            if (status == 0) {
/*              retVal = Py_BuildValue("{s:s,s:s,s:k,s:d,s:O,s:O,s:O,s:O,s:O,s:O,s:O,s:O,s:k,s:O}",
                    "name", name,
                    "type", type,
                    "size", (unsigned long) __size_,
                    "value", (double) __value_,
                    "left", left,
                    "right", right,
                    "ends_with_template", PyBool_FromLong(ends_with_template()),
                    "is_ctor_dtor_conv", PyBool_FromLong(is_ctor_dtor_conv()),
                    "is_reference_or_pointer_to_function_or_array", PyBool_FromLong(is_reference_or_pointer_to_function_or_array()),
                    "is_function", PyBool_FromLong(is_function()),
                    "is_cv_qualifer", PyBool_FromLong(is_cv_qualifer()),
                    "is_array", PyBool_FromLong(is_array()),
                    "list_len", (unsigned long) list_len(),
                    "is_sub", PyBool_FromLong(is_sub())
*/
                retVal = Py_BuildValue("{s:s,s:s,s:k,s:d,s:O,s:O,s:O,s:O}",
                    "name", name,
                    "type", type,
                    "size", (unsigned long) __size_,
                    "value", (double) __value_,
...
__cxa_demangle(const char* mangled_name, char* buf, size_t* n, int* status)
{
    ...
    buf = __demangle(__demangle(mangled_name,
                        static_buf, bs),
                        buf, n, status);
    return buf;
}
...
static PyMethodDef _methods[] = {
    {"_cxa_demangle", _cxa_demangle, METH_VARARGS, "demangle a symbol name"},
    {NULL, NULL, 0, NULL}
};



PyMODINIT_FUNC init_cxa_demangle(void) {
    (void) Py_InitModule("_cxa_demangle", _methods);
}



PyObject* _cxa_demangle(PyObject* self, PyObject* args) {
    PyObject* retVal = NULL;
    const char* mangled_name = NULL;

    if (PyArg_ParseTuple(args, "s", &mangled_name)) {
        __demangle_tree t = __demangle(mangled_name);

        if (t.__root_) {
            PyObject* tree = t.__root_->toPyObject();

            char* name = NULL;
            int status = -1;
            name = __demangle(t, NULL, NULL, &status);
            if (status == 0) {
                retVal = Py_BuildValue("sO", name, tree);
                Py_DECREF(tree);
                free(name);
            }
        }
